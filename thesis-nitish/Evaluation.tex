...
\label{sec:exp_setup}
\todo{Para 1: first learn doc embeddings for all docs. different hyper-parameters to be tuned discussed in backref ~\ref{sec:hp_doc}. what order they tuned in.  learning rate and regcon based on validation. Ref to NCE saying final task decides hp tuning. not perplexity. 
Para 2 : Then categorization using lr algo. HP are learning rate, regcon and logisitic threshold. Again set using validation data.
Para 3 : evaluation critera. F1. Why F1. 
Para 4 : Basilines }
