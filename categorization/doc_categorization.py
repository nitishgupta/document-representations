...
# 	phi_docs = theano.shared(rng.normal(loc=0.0, scale=0.01, size=(len(docs.keys()), K)), name="phi_docs")
# 	phi_cats = theano.shared(rng.normal(loc=0.0, scale=0.01, size=(len(cats.keys()), K)), name="phi_cats")

# 	prob = 1 / (1 + T.exp(- (alpha + b_docs[e1] + b_cats[e2] + T.dot(phi_docs[e1], phi_cats[e2])) ))
# 	prediction = prob > 0.5
# 	cost = truth*T.log(prob) + (1 - truth)*(1 - T.log(prob)) - reg_con*(alpha*alpha + b_docs[e1]*b_docs[e1] + b_cats[e2]*b_cats[e2] + T.dot(phi_docs[e1],phi_docs[e1]) + T.dot(phi_cats[e2],phi_cats[e2]) )

# 	#ga, gb1, gb2, gphi1, gphi2 = T.grad(cost, [alpha, b1, b2, phi1, phi2])
# 	ga, gbd, gbc, gphid, gphic = T.grad(cost, [alpha, b_docs, b_cats, phi_docs, phi_cats])
...
# 																																(phi_docs, phi_docs + lr*gphid), (phi_cats, phi_cats + lr*gphic)) )
...
def mfYelp(trdata, vdata, tdata, K, lr, reg, epoch):
	...
	#sigmoid = 
	#sigmoid = 1 / (1 + T.exp(- (alpha + b_docs[e1] + b_cats[e2] + T.dot(phi_docs[e1], phi_cats[e2]) ) ))
	pre_sigmoid_activation = T.dot(phi_docs[e1], phi_cats[e2])
	sigmoid = T.nnet.sigmoid(pre_sigmoid_activation)
	#sigmoid = 1 / (1 + T.exp(-T.dot(phi_docs[e1], phi_cats[e2])) )

	prediction = T.nnet.sigmoid(pre_sigmoid_activation) > 0.5
	#cost = truth*T.log(sigmoid) + (1 - truth)*(1 - T.log(sigmoid)) - reg_con*(0)
	cost = truth*T.log(T.nnet.sigmoid(pre_sigmoid_activation)) + (1 - truth)*(1 - T.log(T.nnet.sigmoid(pre_sigmoid_activation))) - reg_con*(T.dot(phi_docs[e1],phi_docs[e1]) + T.dot(phi_cats[e2],phi_cats[e2]) )

	#ga, gb1, gb2, gphi1, gphi2 = T.grad(cost, [alpha, b1, b2, phi1, phi2])
	gphid, gphic = T.grad(cost, [phi_docs, phi_cats])

	prob = theano.function(inputs=[e1, e2], outputs = sigmoid)
	predict = theano.function(inputs=[e1, e2], outputs = prediction)
	c = theano.function(inputs=[e1, e2, truth, theano.Param(reg_con, default=reg)], outputs=cost)
	train = theano.function(inputs=[e1, e2, truth, theano.Param(reg_con, default=reg)], outputs=[prediction, cost], updates= ((phi_docs, phi_docs + lr*gphid), (phi_cats, phi_cats + lr*gphic)) )
	...
	# print phi_docs.get_value()[10]
	# print phi_cats.get_value()[15]
...
if __name__=="__main__":
	...
	docs, cats, data = readDocData.readYelp(datafilename);
	...
	mfYelp(trdata, vdata, testdata, K=30, lr=0.01, reg=0.001, epoch=1000)