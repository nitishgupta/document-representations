 	...
 	if(identityWeights <= 0){
		...
		for (a = 0; a < (2*window + 1); a++) {
			for (b = 0; b < embed_size*embed_size; b++) {
	    		next_random = next_random * (unsigned long long)25214903917 + 11;
	    		nn_weight[(a * (embed_size*embed_size)) + b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / (embed_size);
	 		}
	 	}
	}
 	else{
		// Initializing Neural Network Weights to Identity Matrices
		cout<<"Using Identity Weight Matrices\n";
	 	for (a = 0; a < (2*window + 1); a++) {
...
int main(int argc, char **argv){
	...
	if ((i = ArgPos((char *)"-identity-weights", argc, argv)) > 0) identityWeights = atoi(argv[i + 1]);
